        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(LagouSecurity.class)){
                    ...
                    auth = lagouSecurity.value();
                }
                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url),auth);
...
//        method.invoke() //
        ...
        for(Map.Entry<String,String[]> param: parameterMap.entrySet()) {
            ...
            //权限判断
            if(!(Arrays.asList(handler.getAuth()).contains(value))){
                resp.getWriter().write("You are not authorized");
                return;
            }
        }
